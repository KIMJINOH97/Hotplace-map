...
import { Modal, Descriptions, Tag, List } from 'antd';
...
const DetailInfoModal = ({ place }) => {
  ...
  return (
    <>
      <ButtonUtil onClick={() => setVisible(true)} name={'상세보기'} width={'95px'} />
      <Modal
        centered
        visible={visible}
      ...
      >
        <Descriptions title="상세보기" layout="vertical" bordered>
          ...
          {naver_star && (
            <Descriptions.Item label="네이버 별점">{naver_star}</Descriptions.Item>
          )}
          {naver_blog_review_count && (
            <Descriptions.Item label="네이버 블로그 리뷰수">
              {naver_blog_review_count}
            </Descriptions.Item>
          )}
          {naver_buyer_review_count && (
            <Descriptions.Item label="네이버 구매자 리뷰수">
              {naver_buyer_review_count}
            </Descriptions.Item>
          )}
          {kakao_star && (
            <Descriptions.Item label="카카오 별점">{kakao_star}</Descriptions.Item>
          )}
          {instagram_hashtag && (
            <Descriptions.Item label="인스타그램 해시태그수" span={2}>
              {instagram_hashtag}
            </Descriptions.Item>
          )}
