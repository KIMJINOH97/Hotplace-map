...
class Crawler:
    def __init__(self,goo,dong,keyword):
        self.goo = goo
        self.dong  = dong
    ...
    def get_info(self):
        ret =[]
        if self.goo == None or self.dong == None or self.keyword == None:
            print("생성자 ( 구 , 동 , keyword ) 을 확인해주세요")
            return ret
        try:
            search_key = self.goo+" "+self.dong+" "+self.keyword
            driver = webdriver.Chrome(chrome_driver)
            driver.implicitly_wait(5)
            driver.get(url)
            driver.find_element_by_css_selector('#container > shrinkable-layout > div > app-base > search-input-box > div > div > div > input').send_keys(search_key) #검색어 입력하는 부분
            ...
            noneValCnt = 3 #별점이나 주소 전화번호가 3번연속으로 안나왔을 경우 ,특정 동기준 데이터의 끝부분이란점을 이용하여 종료
            ...
            for j in range(0,6):
                ...
                for i in range(1,51):
                    ...
                    try:
                        ...
                        if store_address.find(self.dong) == -1:
                            print(self.dong + " 데이터가 나오지 않습니다.")
                            raise Exception
